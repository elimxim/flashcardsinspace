...
describe('review-store media', () => {
  ...
  it('should not let a slow fetch paint its media onto a later flashcard', async () => {
    // the first flashcard's audio never arrives until we say so
    ...
    // the user advances twice before that audio lands
  ...
  it('should leave nothing behind when destroyed', () => {
    ...
    // the state entry is released, and the instance is gone too - so the next
  ...
  it('should not let a revisited flashcard overwrite the one now on screen', async () => {
    ...
    // the user steps back and forward again before the step back has settled.
    ...
    // that abandoned fetch must not write anything, or it would wipe the
