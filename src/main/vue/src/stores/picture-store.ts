...
export const usePictureStore = defineStore('picture', {
  ...
  actions: {
    ...
    setPictureId(flashcardId: number, side: string, pictureId: number) {
      Log.log(
        LogTag.STORE,
        `picture.setPictureId: Picture.id=${pictureId}, Flashcard.id=${flashcardId}, Flashcard.side=${side}`,
      )
    ...
    getPictureId(flashcardId: number | undefined, side: string): number | undefined {
      Log.log(
        LogTag.STORE,
        `picture.getPictureId: Flashcard.id=${flashcardId}, Flashcard.side=${side}`,
      )
    ...
    removePictureId(flashcardId: number, side: string) {
      Log.log(
        LogTag.STORE,
        `picture.removePictureId: Flashcard.id=${flashcardId}, Flashcard.side=${side}`,
      )
      this.checkStateLoaded()
      const key = pictureMapKey(flashcardId, side)
...
const pictureMapKey: (flashcardId: number, side: string) => string = (flashcardId, side) =>
  `${flashcardId}-${side}`
